...
function OTP() {
  ...
  const handleOTPVerification = async (e) => {
    ...
    try {
      ...
      // Redirect if OTP is verified successfully
      if (response.data.success) {
        window.location.href = "/";  // Change this to the reset password page route
      }
    } catch (error) {
    ...
    <div className="container mx-auto min-h-screen flex flex-col md:flex-row">
      ...
      <div className="w-full md:w-1/2 bg-cover bg-center flex flex-col justify-center px-6 md:px-20 py-10" style={{ backgroundImage: `url(${LoginBg})` }}>
        ...
        <div className="md:mt-0 lg:mt-8 w-full max-w-sm mx-auto">
          ...
          <form className="space-y-6 w-full" onSubmit={handleOTPVerification}>
            <div>
              <input
                type="text"
                name="otp"
